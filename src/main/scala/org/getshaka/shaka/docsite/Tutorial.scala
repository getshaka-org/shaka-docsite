...
//      p{t"""Using ${a{href("https://dotty.epfl.ch/docs/reference/contextual/context-functions.html#example-builder-pattern"); target("_blank"); t"Context Functions"}} in this way allows Shaka to efficiently build and manage Bindings."""}
      ...
      h3WithAnchor("passing-data-through-constructors", "Passing Data Through Constructors").render
      ...
      h3WithAnchor("making-an-interactive-component", "Making an Interactive Component").render
      ...
      TicTac3().render
      hr{}
      p{t"""onclick will assign its parameter to the onclick property of the parent <button> Element. If you click a Square now, you should see a browser alert."""}
      ...
      p{t"""SquareValue is a Scala 3 enum, being stored in an immutable Array. We can now create the GameState object, with all-empty squareValues and X going first:"""}
      pre{
        code{cls("scala doc-code")
      ...
      p{t"'t' can accept Frags, Components, builder methods, and objects of any type. It works great on multi-line strings, automatically stripping the '|' margin at compile time. t is also available as an extension method on String."}
      ...
      h3WithAnchor("storing-a-history-of-moves", "Storing a History of Moves").render
      ...
      p{t"A final cool property about State is that you can easily persist it, whether in LocalStorage, SessionStorage, or any other scheme implementing the StorageManager trait. First, add an instance of the desired StorageManager to State's constructor."}
      pre{
        code{cls("scala doc-code")
          t"""object GameState extends State(InitialGameState, LocalStorage("game-state"))"""
        }
      }
      p{t"""LocalStorage uses a given NativeConverter typeclass to serialize Game to a JSON String and back. NativeConverter supports ${a{href("https://dotty.epfl.ch/docs/reference/contextual/derivation.html"); target("_blank"); t"Typeclass derivation"}}, making it easy to generate an implementation."""}
      pre{
        code{cls("scala doc-code")
          t"""case class Game(
             |  xIsNext: Boolean,
